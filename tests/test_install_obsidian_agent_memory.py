...
class InstallerHomeSkillTests(unittest.TestCase):
    ...
    def test_install_packet_workspace_inits_then_preserves_existing_vault_state(self) -> None:
        ...
        self.assertIn("Wiki Overview", overview_path.read_text(encoding="utf-8"))
        self.assertTrue((vault / "wiki" / "queries" / ".gitkeep").exists())
        self.assertTrue((vault / "wiki" / "tours" / ".gitkeep").exists())
        self.assertTrue((vault / "wiki" / "meta" / ".gitkeep").exists())
        self.assertTrue((vault / "wiki" / "meta" / "dashboard.base").exists())
        self.assertTrue((vault / "wiki" / "meta" / "diff-impact.md").exists())
        self.assertTrue((vault / "raw" / "imports" / ".gitkeep").exists())
        self.assertTrue((vault / "raw" / "converted" / ".gitkeep").exists())
        self.assertTrue((vault / ".llm-wiki" / "state" / ".gitkeep").exists())
        self.assertTrue((vault / ".llm-wiki" / "reports" / ".gitkeep").exists())
        ...
        self.assertEqual(config["obsidian"]["known_local_vault_roots"], [])
        ...
        self.assertEqual(config["wiki_layer"]["known_local_vault_roots"], [])
        ...
        self.assertEqual(config["wiki_layer"]["overview_path"], "wiki/overview.md")
        self.assertEqual(config["wiki_layer"]["queries_path"], "wiki/queries")
        self.assertEqual(config["wiki_layer"]["tours_path"], "wiki/tours")
        self.assertEqual(config["wiki_layer"]["compile_state_path"], ".llm-wiki/state/wiki-compile.json")
        self.assertEqual(config["wiki_layer"]["reports_path"], ".llm-wiki/reports")
        self.assertEqual(config["wiki_layer"]["conflict_policy"], "flag")
        self.assertTrue(config["wiki_layer"]["saved_queries_enabled"])
        self.assertTrue(config["wiki_layer"]["deep_research_save_default"])
        self.assertEqual(config["wiki_layer"]["note_types"], ["synthesis", "concept", "source", "decision", "session", "query"])
        self.assertEqual(config["wiki_layer"]["folders"]["queries"], "wiki/queries")
        self.assertTrue(config["retrieval_router"]["enabled"])
        self.assertEqual(config["retrieval_router"]["max_hops"], 3)
        self.assertFalse(config["wiki_compile"]["enabled"])
        self.assertFalse(config["memory_capture"]["enabled"])
