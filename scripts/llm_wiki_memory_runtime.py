...
def default_runtime_settings(workspace_root: Path, config_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "brv_command": str(byterover.get("command") or ""),
...
def ensure_brv_command(runtime: dict[str, Any], summary: list[str], failures: list[str], state: dict[str, Any]) -> str | None:
    ...
    existing_candidate = first_existing(local_candidates)
    if existing_candidate:
        summary.append(f"brv optional integration: resolved existing managed candidate {existing_candidate}")
        return str(existing_candidate)

    configured_command = str(runtime["brv_command"] or "").strip()
    if configured_command:
        configured_path = Path(configured_command).expanduser()
        if configured_path.exists() or command_in_path(configured_command):
            summary.append(f"brv optional integration: resolved existing command {configured_command}")
            return configured_command
        summary.append(
            f"brv optional integration unavailable: configured command not found ({configured_command}); using wiki/local memory only"
        )
        return None

    summary.append("brv optional integration not configured; using wiki/local memory only")
    return None
